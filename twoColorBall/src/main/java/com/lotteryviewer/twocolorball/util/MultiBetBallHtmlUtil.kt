...
object MultiBetBallHtmlUtil {
    ...
    private const val getHistoryBallStr =
        ("javascript:function getHistoryBallStr() {"
                ...
                + "    var trs = l.getElementsByTagName('tr');"
                + "    // 计算有多少行数据，确定要循环多少次"
                ...
                + "        var tr = trs[i];"
                + "        // 得到每一次开奖日期"
                + "        var balls += tr.getElementsByTagName('td')[1].innerText;"
                + "        var balls += '${MultiBetBallDataUtil.SPLIT_INNER}';"
                + "        for (var j = 0; j < 7; j++) {"
                + "            // 累加每行的前 7 个 span 内的文本，即 6 个红球和 1 个篮球字符串"
                + "            balls += tr.getElementsByTagName('span')[j].innerText;"
                + "            if (j < 6) {"
                + "                // 每一组的 7 个球之间使用 _ 下划线分割，最后一个号码除外"
                + "                balls += '${MultiBetBallDataUtil.SPLIT_INNER}';"
                + "            }"
                + "        }"
                + "        // 每一组之间使用 ; 分号分割"
                + "        balls += '${MultiBetBallDataUtil.SPLIT_GAP}';"
                + "    }"
                + "    return balls;"
                + "}")
